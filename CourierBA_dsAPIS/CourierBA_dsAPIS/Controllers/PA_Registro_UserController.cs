...
{
    public class PA_Registro_UserController : ApiController
    {
        //Empresa
        [HttpGet]
        public int getCreateUser(string Nombre, string Apellido, string Telefono, string Correo)
        {
            int messege = 0;
            ...
            using (var connection = Connection.ConnectionSql.getConnection())
            {
                ...
                using (SqlCommand command = new SqlCommand("PA_Registro_User", connection))
                {
                    try
                    {
                        ...
                        command.Parameters.Add("@pNombre", SqlDbType.VarChar).Value = Nombre;
                        command.Parameters.Add("@pApellido", SqlDbType.VarChar).Value = Apellido;
                        command.Parameters.Add("@pCelular", SqlDbType.VarChar).Value = Telefono;
